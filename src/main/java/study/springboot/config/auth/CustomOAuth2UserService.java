package study.springboot.config.auth;
...
import study.springboot.config.auth.dto.OAuthAttributes;
import study.springboot.config.auth.dto.SessionUser;
import study.springboot.domain.user.User;
import study.springboot.domain.user.UserRepository;
...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException{
        ...
        /*
        registrationId
            현재 로그인 진행 중인 서비스(예를 들어 구글, 네이버 로그인)를 구분하는 코드
